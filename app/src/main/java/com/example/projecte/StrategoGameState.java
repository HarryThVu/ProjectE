...
public class StrategoGameState {
            ...
            case 1:  //aka "up"
                if (chosenY - 1 >= 0 && gameboard[chosenX][chosenY - 1] == null) {
                    ...
                    if (opponentRank > chosen.getRank()) {
                        ...
                        legal = true;
                        whatsUp = whatsUp + "\nPlayer " + playerID + " has lost their " + chosen.getRank() + ".";
                    } else {
                        ...
                        legal = true;
                        whatsUp = whatsUp + "\nPlayer " + playerID +
                                " has triumphed in a battle with their " + chosen.getRank() + ".";
                    }
                } else {
                    whatsUp = whatsUp + "\nUnit " + chosen.getRank() + " can't move there.";
                    legal = false;
                }
                break;
            //End case 1
    ...
    public boolean selectPiece(int playerID, Unit chosenP) {
        if (chosenP.getOwnerID() == playerID){
            ...
            whatsUp = whatsUp + "\nPlayer " + playerID + " selected their " + chosenP.getRank() + ".";
            return true;
        }
        else {
    ...
    public boolean placePiece(int playerID, Unit unit, int x, int y) {
        if (unit.getStatus()) {
            if (playerID == 0 && y < 4) {  //< 4 is for boundary purposes, ensures piece is on your side
                ...
                unit.setxLoc(y);
                gameboard[x][y] = unit;
                whatsUp = whatsUp + "\nPlayer " + playerID + " has placed their " + unit.getRank() + ".";
                return true;
            }
            ...
            whatsUp = whatsUp + "\nPlayer " + playerID + " tried to hit a ghost...?";
            return false;
        }
    }//placePiece
